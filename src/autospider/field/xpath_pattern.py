...
class FieldXPathExtractor:
    ...
    def extract_common_pattern(
        ...
        print(f"[FieldXPathExtractor] 从 {len(records)} 条记录中提取字段 '{field_name}' 的公共 XPath...")
        ...
        if not xpaths:
            print(f"[FieldXPathExtractor] ⚠ 未找到有效的 XPath")
            return None
        ...
        if not common_pattern:
            print(f"[FieldXPathExtractor] ⚠ 未找到公共 XPath 模式")
            return None
