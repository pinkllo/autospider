...
class ArtifactPlanRepository:
    ...
    def save_plan(self, plan: TaskPlan) -> TaskPlan:
        ...
        logger.info("[Planner] 任务计划已成功持久化至: %s", self._output_dir / "task_plan.json")
    ...
    def write_knowledge_doc(self, plan: TaskPlan) -> None:
        ...
        if not content:
            return
        doc_path = self._output_dir / "plan_knowledge.md"
    ...
    def build_knowledge_doc(self, plan: TaskPlan) -> str:
        ...
        domain = urlparse(self._site_url).netloc
        ...
        lines.append("")
        lines.append("## 站点概况")
        lines.append(f"- URL: {self._site_url}")
        lines.append(f"- 需求: {self._user_request}")
