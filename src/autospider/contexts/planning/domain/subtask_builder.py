...
class PlannerSubtaskBuilder:
    ...
    def _build_variant_label(self, context: dict[str, str] | None) -> str | None:
        label = self._planner._format_context_path(context)
    ...
    def _build_subtasks_from_variants(
        ...
        *,
        ...
        for idx, variant in enumerate(variants):
            ...
            sanitized_context = self._planner._sanitize_context(variant.context)
            scope = self._build_subtask_scope(raw=raw, context=sanitized_context)
            task_desc = str(raw.get("task_description") or "").strip() or self._build_task_description_for_mode(
                sanitized_context,
                mode,
            )
    ...
    def _build_subtask_scope(
        ...
        *,
        ...
        category_path = self._planner._extract_category_path(context)
        scope_key = str(raw.get("scope_key") or "").strip()
        scope_label = str(raw.get("scope_label") or "").strip()
        if not scope_key and category_path:
            normalized = [
                self._planner._normalize_semantic_label(item) for item in category_path if item
            ]
    ...
    def _build_subtask_id(
        ...
        *,
        ...
        base = (
            ...
            or self._planner._format_context_path(context)
    ...
    def _build_expand_task_description(self, context: dict[str, str] | None) -> str:
        scope = self._planner._format_context_path(context)
    ...
    def _resolve_requested_count_text(self, prefix: str = "各") -> str:
        ...
        match = re.search(r"(\d+)\s*条", str(self._planner.user_request or ""))
        if match:
    ...
    def _register_sibling_categories(self, subtasks: list[SubTask]) -> None:
        registry = self._planner._get_sibling_category_registry()
        for subtask in subtasks:
            category_path = self._planner._extract_category_path(subtask.context)
            parent_signature = self._planner._build_parent_category_signature(category_path)
            leaf_label = self._planner._normalize_category_leaf_label(
