...
class PlannerAnalysisPostProcessor:
    ...
    def _post_process_analysis(
        ...
        *,
        ...
        current_context = self._planner._sanitize_context(node_context)
        ...
        if not self._should_split_by_category():
            return normalized
        if self._planner._extract_category_path(current_context):
            return normalized
        ...
        if not fact_subtasks:
            ...
            return self._planner._append_observation_note(
                normalized,
        ...
        return self._planner._append_observation_note(normalized, note)
    ...
    def _prune_backtrack_subtasks(
        ...
        current_path = self._planner._extract_category_path(context)
        if not subtasks or not current_path:
            return normalized

        current_path_norm = [
            self._planner._normalize_semantic_label(item) for item in current_path if item
        ]
        ...
        for item in subtasks:
            ...
            candidate_path = self._planner._expand_category_segments(name)
            candidate_path_norm = [
                self._planner._normalize_semantic_label(segment)
        ...
        if not str(normalized.get("task_description") or "").strip():
            normalized["task_description"] = self._planner._build_collect_task_description(context)
        return self._planner._append_observation_note(normalized, note)
    ...
    def _collapse_sibling_switches_to_leaf(
        ...
        current_label = self._planner._get_current_category_label(context)
        ...
        display_label = self._planner._strip_category_group_prefix(current_label)
        normalized["page_type"] = "list_page"
        self._set_analysis_candidates(normalized, [])
        normalized["task_description"] = self._planner._build_collect_task_description(
            self._planner._build_subtask_context(display_label, context)
        )
        note = (
    ...
    def _should_split_by_category(self) -> bool:
        resolver = getattr(self._planner, "_get_grouping_semantics", None)
    ...
    def _build_page_fact_subtasks(self, analysis: dict) -> list[dict]:
        ...
        for raw in self._get_page_fact_candidates(analysis):
            ...
            task_description = str(candidate.task_description or "").strip() or self._build_category_candidate_task_description(label)
            subtasks.append(
                {
                    "name": label,
    ...
    def _matches_requested_categories(self, label: str, requested_categories: list[str]) -> bool:
        candidate_tokens = {
            self._planner._normalize_semantic_label(label),
            self._planner._normalize_category_leaf_label(label),
            *[
                self._planner._normalize_semantic_label(item)
                for item in self._planner._expand_category_segments(label)
            ],
        }
        candidate_tokens = {item for item in candidate_tokens if item}
        for requested in requested_categories:
            requested_tokens = {
                self._planner._normalize_semantic_label(requested),
                self._planner._normalize_category_leaf_label(requested),
                *[
                    self._planner._normalize_semantic_label(item)
                    for item in self._planner._expand_category_segments(requested)
                ],
            }
    ...
    def _build_candidate_scope(
        ...
        scope_context = self._planner._build_subtask_context(label)
        scope_path = self._planner._extract_category_path(scope_context)
        scope_key = str(candidate.scope_key or "").strip()
        if not scope_key:
            normalized = [
                self._planner._normalize_semantic_label(item) for item in scope_path if item
            ]
    ...
    def _build_category_candidate_task_description(self, category_name: str) -> str:
        return self._planner._build_expand_task_description(
            self._planner._build_subtask_context(category_name)
        )
    ...
    def _looks_like_sibling_switch_group(
        ...
        current_group, current_leaf = self._planner._split_grouped_category_label(current_label)
        if not current_group or not current_leaf:
            return False

        normalized_current_group = self._planner._normalize_semantic_label(current_group)
        normalized_current_leaf = self._planner._normalize_semantic_label(current_leaf)
        if not normalized_current_group or not normalized_current_leaf:
            return False

        has_peer_candidate = False
        for name in candidate_names:
            group, leaf = self._planner._split_grouped_category_label(name)
            normalized_group = self._planner._normalize_semantic_label(group)
            normalized_leaf = self._planner._normalize_semantic_label(leaf)
    ...
    def _matches_registered_sibling_switches(
        ...
        parent_signature = self._planner._build_parent_category_signature(current_path)
        registered = self._planner._get_sibling_category_registry().get(parent_signature) or set()
        if len(registered) < 2:
            return False

        candidate_labels: set[str] = set()
        for name in candidate_names:
            normalized = self._planner._normalize_category_leaf_label(name)
            if normalized:
                candidate_labels.add(normalized)
        if not candidate_labels:
            return False

        current_label = self._planner._normalize_category_leaf_label(current_path[-1])
    ...
    def _looks_like_current_category(self, name: str, analysis: dict) -> bool:
        ...
        if not current_selected:
            return False
        normalized_label = self._planner._normalize_category_leaf_label(label)
        normalized_selected = self._planner._normalize_category_leaf_label(current_selected)
