...
class AdaptiveRateController:
    ...
    def __init__(
        ...
        self.credit_recovery_pages = (
            credit_recovery_pages or config.url_collector.credit_recovery_pages
        )
    ...
    def apply_penalty(self) -> None:
        ...
        if self.current_level < self.max_level:
            ...
            logger.info(
                f"[速率控制] ⚠ 触发惩罚，降速等级提升至 {self.current_level}/{self.max_level}"
            )
            logger.info(
    ...
    def _try_credit_recovery(self) -> None:
        ...
        if self.current_level > 0:
            self.current_level -= 1
            logger.info(
                f"[速率控制] ✓ 信用恢复，降速等级降至 {self.current_level}/{self.max_level}"
            )
