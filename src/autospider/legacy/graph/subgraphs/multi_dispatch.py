...
def _resolve_subtask_target_url_count(subtask, params: dict[str, Any]) -> int | None:
    ...
    if getattr(
        subtask, "per_subtask_target_count", None
    ) is not None and _is_grouped_category_subtask(subtask):
...
def _build_dispatch_summary(
    plan: TaskPlan, subtask_results: list[SubTaskRuntimeState]
) -> dict[str, Any]:
...
async def run_subtask_worker_node(state: SubTaskFlowState):
    ...
    while True:
        try:
            worker = SubTaskWorker(
                ...
                task_plan_snapshot=(
                    plan.model_dump(mode="python") if isinstance(plan, TaskPlan) else {}
                ),
                plan_journal=(
                    [
    ...
    return {
        ...
        "round_expand_requests": (
            [dict(result.get("expand_request") or {})] if result.get("expand_request") else []
        ),
        "artifacts": artifacts,
    }
...
async def complete_dispatch(state: MultiDispatchState) -> MultiDispatchState:
    ...
    if has_pending:
        ...
        return {
            "node_status": "fatal",
            "node_error": {"code": "taskplane_incomplete", "message": message},
            "error": {"code": "taskplane_incomplete", "message": message},
        }
    mutation = PlanMutationService().merge_expand_requests(
        plan=plan,
        expand_requests=[],
...
def build_multi_dispatch_subgraph():
    ...
    builder.add_conditional_edges(
        "prepare_dispatch_batch", route_dispatch_batch, {"complete_dispatch": "complete_dispatch"}
    )
