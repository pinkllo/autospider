...
class FieldXPathExtractor:
    ...
    def _smart_extract_common_pattern(self, xpaths: list[str]) -> str | None:
        ...
        if len(set(segment_counts)) > 1:
            ...
            # 过滤出相同长度的XPath
            filtered_xpaths = [
                xpaths[i] for i, segs in enumerate(all_segments) if len(segs) == most_common_count
            ]
        ...
        for seg_idx in range(num_segments):
            ...
            if not keep_index and not common_attrs:
                class_enhanced_attr = self._find_common_class_for_position(all_segments, seg_idx)
    ...
    def _calculate_pattern_confidence(self, original_xpaths: list[str], pattern: str) -> float:
        ...
        if "|" in pattern:
            ...
            pattern_parts = [
                self._normalize_for_comparison(p.strip()) for p in pattern.split("|") if p.strip()
            ]
    ...
    def _fallback_extract_pattern(self, xpaths: list[str]) -> str | None:
        ...
        logger.info(f"[FieldXPathExtractor] (回退)最常见模式: {_escape_markup(common_pattern)}")
        logger.info(
            f"[FieldXPathExtractor] (回退)出现次数: {count}/{len(xpaths)} (置信度: {confidence:.2%})"
        )
