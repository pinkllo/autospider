...
class BatchXPathExtractor:
    ...
    def __init__(
        ...
        self.required_fields = {rule.name: rule.required for rule in self.field_rules if rule.name}
    ...
    def _select_best_candidate(self, candidates: list[str], data_type: str) -> str | None:
        ...
        if len(scored) > 1:
            ...
            if (top_score - second_score) < 1.0 and self._normalize_text(
                top_value
            ) != self._normalize_text(second_value):
                return None
