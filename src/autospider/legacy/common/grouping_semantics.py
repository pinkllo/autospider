...
def normalize_grouping_semantics(payload: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    category_discovery_mode = normalize_category_discovery_mode(raw.get("category_discovery_mode"))
...
def normalize_semantic_labels(value: Any) -> list[str]:
    ...
    if isinstance(value, (list, tuple, set)):
        ...
        items = [value]
    labels = {str(item or "").strip() for item in items if str(item or "").strip()}
