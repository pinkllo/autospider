                    ...
                    "normalized_result": {
                        ...
                        "task_overrides": {
                            ...
                            "per_group_target_count": (
                                result.task.per_group_target_count if result.task else None
                            ),
                            "total_target_count": (
                                result.task.total_target_count if result.task else None
                            ),
                            "category_discovery_mode": (
                                result.task.category_discovery_mode if result.task else "auto"
                            ),
                            "requested_categories": (
                                result.task.requested_categories if result.task else []
                            ),
                            "category_examples": (
                                result.task.category_examples if result.task else []
                            ),
                            "max_pages": result.task.max_pages if result.task else None,
                            "target_url_count": (
                                result.task.target_url_count if result.task else None
                            ),
                            "consumer_concurrency": (
                                result.task.consumer_concurrency if result.task else None
                            ),
                            "field_explore_count": (
                                result.task.field_explore_count if result.task else None
                            ),
                            "field_validate_count": (
                                result.task.field_validate_count if result.task else None
                            ),
                        },
                    },
                },
    ...
    def _normalize_result(
        ...
        if status == "ready":
            ...
            if task is None:
                status = "need_clarification"
                next_question = (
                    next_question or "我还缺少可执行信息。请提供列表页 URL，并确认希望提取的字段。"
                )
