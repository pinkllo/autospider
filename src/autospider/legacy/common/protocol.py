...
def _summarize_response_shape(value: Any, *, depth: int = 0, max_depth: int = 5) -> Any:
    ...
    if isinstance(value, Mapping):
        ...
        for key in list(value.keys())[:12]:
            result[str(key)] = _summarize_response_shape(
    ...
    if isinstance(value, Sequence) and not isinstance(value, (str, bytes, bytearray)):
        ...
        return {
            ...
            "length": len(value),
            "items": [
                _summarize_response_shape(item, depth=depth + 1, max_depth=max_depth)
                for item in items
            ],
        }
    if hasattr(value, "model_dump") and callable(value.model_dump):
        try:
            return {
                "__type__": type(value).__name__,
                "model_dump": _summarize_response_shape(
                    value.model_dump(mode="python"), depth=depth + 1, max_depth=max_depth
                ),
            }
        except Exception:
            pass
    ...
    for attr in ("text", "content", "arguments", "additional_kwargs", "response_metadata"):
        ...
        if attr_value is not None:
            summary[attr] = _summarize_response_shape(
