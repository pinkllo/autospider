...
class PgColdStore:
    ...
    async def get_envelope(self, envelope_id: str) -> PlanEnvelope | None:
        row = await self._fetch_one(
            self._select_table(self._envelope_table(), envelope_id=envelope_id)
        )
        if row is None:
            return None
    ...
    async def save_ticket(self, ticket: TaskTicket) -> None:
        await self._execute_upsert(
            self._ticket_table(), self._ticket_payload(ticket), ("ticket_id",)
        )
    ...
    async def get_tickets_by_envelope(self, envelope_id: str) -> list[TaskTicket]:
        query = self._ordered_ticket_query().where(
            self._ticket_table().c.envelope_id == envelope_id
        )
    ...
    async def claim_next(
        ...
        return [
            ...
            if (ticket := await self.get_ticket(ticket_id)) is not None
        ]
    ...
    def _ticket_from_row(
        self, row: dict[str, Any], *, result: TaskResult | None = None
    ) -> TaskTicket:
    ...
    async def _execute_upsert(
        self, table, payload: dict[str, Any], conflict_keys: tuple[str, ...]
    ) -> None:
    ...
    async def _execute_update(self, ticket_id: str, payload: dict[str, Any]) -> None:
        ...
        async with engine.begin() as conn:
            await conn.execute(
                update(table).where(table.c.ticket_id == ticket_id).values(**payload)
            )
    ...
    def _select_table(self, table, **filters: Any):
        query = (
            self._ordered_ticket_query()
            if table is self._ticket_table()
            else self._sqlalchemy("select")[0](table)
        )
