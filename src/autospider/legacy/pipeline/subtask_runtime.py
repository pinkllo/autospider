...
def _build_runtime_summary(run_result: PipelineRunResult) -> dict[str, Any]:
    ...
    return {
        ...
        "reliable_for_aggregation": _is_reliable(
            {"summary": run_result.to_payload(), "outcome_type": outcome_type}
        ),
    ...
    return SubTaskRuntimeState.model_validate(
        {
            ...
            "execution_brief": restore_execution_brief(subtask.execution_brief).model_dump(
                mode="python"
            ),
...
def build_dispatch_summary(
    plan: TaskPlan, runtime_states: list[SubTaskRuntimeState]
) -> dict[str, Any]:
