...
class TaskScheduler:
    ...
    async def submit_envelope(self, envelope: PlanEnvelope) -> SubmitReceipt:
        ...
        queued_tickets = [self._queue_ticket(ticket, envelope.envelope_id) for ticket in envelope.tickets]
    ...
    async def cancel_ticket(self, ticket_id: str, reason: str = "") -> None:
        ...
        if ticket is None or ticket.status.is_terminal:
            return
        cancelled = await self._store.update_status(ticket_id, TicketStatus.CANCELLED, assigned_to=None)
