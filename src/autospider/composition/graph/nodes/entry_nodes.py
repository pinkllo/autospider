...
async def chat_clarify(state: dict[str, Any]) -> dict[str, Any]:
    ...
    max_turns = _to_positive_int(
        _conversation_value(state, "chat_max_turns"), _to_positive_int(cli_args.get("max_turns"), 6)
    )
...
async def chat_review_task(state: dict[str, Any]) -> dict[str, Any]:
    ...
    action_payload = (
        review_payload if isinstance(review_payload, dict) else {"action": review_payload}
    )
...
async def chat_history_match(state: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        state.get("history_match_done")
        and str(state.get("history_match_signature") or "") == task_signature
    ):
