    ...
    *,
    ...
    def _save() -> None:
        ...
        payload = TaskRunPayload(
            ...
            field_names=[
                str(field.name or "").strip() for field in fields if str(field.name or "").strip()
            ],
...
def _merge_extraction_configs(existing: dict[str, Any], incoming: dict[str, Any]) -> dict[str, Any]:
    ...
    def _consume(config: dict[str, Any]) -> None:
        for raw_field in list(dict(config or {}).get("fields") or []):
            ...
            seen: set[str] = (
                {str(candidate.get("xpath") or "").strip()}
                if str(candidate.get("xpath") or "").strip()
                else set()
            )
        ...
        try:
            await finalizer.finalize(
                PipelineFinalizationContext(
                    ...
                    failure_records=[
                        dict(item) for item in list(runtime_context.failure_records or ())
                    ],
...
async def _process_task(
    ...
    if _is_finalized_run_record(existing_record):
        ...
        if tracker:
            if (
                existing_record.get("success")
                and existing_record.get("durability_state") == "durable"
            ):
                await tracker.record_success(url)
