...
class RedisHotStore:
    ...
    async def save_envelope(self, envelope: PlanEnvelope) -> None:
        client = self._client()
        await client.hset(
            self._envelope_key(envelope.envelope_id), mapping={"blob": encode_model(envelope)}
        )
    ...
    async def save_result(self, result: TaskResult) -> None:
        client = self._client()
        await client.hset(
            self._result_key(result.ticket_id), mapping={"blob": encode_model(result)}
        )
        ticket = await self.get_ticket(result.ticket_id)
        if ticket is None:
            return
        await self._write_ticket(
            ticket.model_copy(update={"result": result, "updated_at": result.completed_at})
        )
    ...
    async def delete_ticket(self, ticket_id: str) -> None:
        ...
        await client.delete(
            self._ticket_key(ticket_id), self._result_key(ticket_id), self._running_key(ticket_id)
        )
