...
class _CliRuntimeProxy:
    """Lazily load cli_runtime so lightweight CLI paths stay import-safe."""
...
def _benchmark_reports_dir() -> Path:
    return _repo_root() / "output" / "benchmark" / "reports"


def _benchmark_json_reports() -> list[Path]:
    return sorted(_benchmark_reports_dir().glob("*.json"))


def _benchmark_paths() -> dict[str, Path]:
    root = _repo_root() / "tests" / "benchmark"
    return {
        "benchmark": root,
...
def _latest_benchmark_report_path() -> Path:
    reports = _benchmark_json_reports()
    if not reports:
        raise FileNotFoundError(f"No benchmark reports found in {_benchmark_reports_dir()}")
    return reports[-1]
...
def _last_two_benchmark_reports() -> tuple[Path, Path]:
    ...
    if len(reports) < 2:
        raise FileNotFoundError(
            f"Need at least two benchmark reports in {_benchmark_reports_dir()}"
        )
...
def _load_benchmark_runner() -> type:
    module = importlib.import_module("tests.benchmark.runner")
    return getattr(module, "BenchmarkRunner")
...
def _build_benchmark_runner(base_url: str) -> Any:
    ...
    runner_cls = _load_benchmark_runner()
    return runner_cls(
...
def _list_benchmark_scenarios() -> list[str]:
    ...
    return list_scenarios(_benchmark_paths()["scenarios"])
...
def _compare_latest_benchmark_reports() -> dict[str, Any]:
    ...
    return reporter.compare_reports(previous_path, latest_path)
...
def _compare_new_benchmark_report(new_report_path: Path) -> dict[str, Any]:
    ...
    reporter = _load_benchmark_reporter()
    return reporter.compare_reports(existing_reports[-1], new_report_path)
...
def _write_benchmark_reports(results: dict[str, Any]) -> tuple[Path, Path]:
    reporter = _load_benchmark_reporter()
    reports_dir = _benchmark_reports_dir()
    report_name = _benchmark_report_stem()
...
def _benchmark_report_stem() -> str:
...
def benchmark_command(
    ...
    scenario: tuple[str, ...] = typer.Option(
        (),
        "--scenario",
        "-s",
