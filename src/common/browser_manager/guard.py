...
class PageGuard:
    ...
    def attach_to_page(self, page: Page):
        ...
        page.on(
            "framenavigated",
            lambda frame: (
                asyncio.create_task(self.run_inspection(page)) if should_inspect(frame) else None
            ),
        )

        # 监听 DOM 内容加载（捕获弹窗式登录）
        page.on("domcontentloaded", lambda: asyncio.create_task(self.run_inspection(page)))
