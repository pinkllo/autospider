...
async def load_file(
    file_path: Union[str, Path], as_json: bool = False, encoding: str = "utf-8"
...
async def copy_file(src: Union[str, Path], dst: Union[str, Path], overwrite: bool = False) -> bool:
    """
    异步复制文件
...
async def move_file(src: Union[str, Path], dst: Union[str, Path], overwrite: bool = False) -> bool:
    """
    异步移动/重命名文件
...
async def calculate_file_hash(
    file_path: Union[str, Path], algorithm: str = "md5", chunk_size: int = 8192
) -> Optional[str]:
    """
    异步计算文件哈希值
...
async def is_same_file(path_a: Union[str, Path], path_b: Union[str, Path]) -> bool:
    ...
    try:
        ...
        # 比较哈希值
        hash_a, hash_b = await asyncio.gather(calculate_file_hash(p1), calculate_file_hash(p2))
